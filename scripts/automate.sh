...
for ((i=1;i<=$servers;i+=1)); do
	s=`printf "%02d" $i`;
	echo net$s
	echo "ssh to net$s and run ./server"
	ssh net$s "cd $ROOT_DIR; nohup ./server > output/s_$s 2>&1 &"
done
for ((;i<=$total;i+=1)); do
	s=`printf "%02d" $i`;
	echo net$s
	echo "ssh to net$s and run ./client"
	ssh net$s "cd $ROOT_DIR; nohup ./client > output/c_$s 2>&1 &"
done
